...
== Setup
Make sure the `hivemq extension sdk` repository is checked out in the same folder as the customization sdk, so the hivemq
extension sdk can be linked with this repository (see `settings.gradle.kts` file line `includeBuild("../hivemq-extension-sdk")`).
